...
here (found in the `beam-sqlite` package).
...
`StandaloneDeriving` extension to derive an instance of `Show` and `Eq` for the 'regular' datatype.
...
We use the applicative `<$>` operator to simplify our definiton for
`primaryKey`, relying on the applicative instance for `(a ->)`.
